"""
MedSAM2 zero-shot evaluation (P1) — volume-level, oracle box + propagation.

MedSAM2 is promptable, not few-shot, so this is a separate harness from test.py.
It reuses the *identical* scorer (eval_common.Scores) and per-organ + MEAN
aggregation so its numbers sit in the same table as the prototype baseline.

Protocol (per organ, per query volume):
  - take the query GT for that organ; FG slices = slices where the organ appears
    (same scoring stack as test.py).
  - build an ORACLE box from the GT on the prompt slice(s) — this is an UPPER BOUND
    ("given a perfect localizer"), analogous to the 'percentile' oracle of the
    baseline. It is NOT deployable; the deployable box-from-FSS variant is P3.
  - crop to the FG depth range, prompt, propagate both directions, score on fg_idx.

prompt modes:
  perslice (default) — an oracle box on every FG slice (robust upper bound).
  key                — one box on the largest-area FG slice + bidirectional
                       propagation (MedSAM2's native single-prompt usage).

Image normalization is MedSAM2's (uint8 [0,255] + 512 + ImageNet), NOT the z-score
