...
"""
3D Felzenszwalb graph-based segmentation.

Ported verbatim from the original Q-Net / SSL-ALPNet supervoxel kernel
(data/supervoxels/felzenszwalb_3d_cy.pyx). The only changes relative to the
original are mechanical, behaviour-preserving ones so it builds against a
modern toolchain (Cython 3.x / NumPy 2.x):

  * the union-find helpers (find_root / set_root / join_trees) are inlined
    here instead of being cimported from skimage's internal _ccomp module;
    the bodies are copied verbatim from _ccomp.pyx.
  * raw buffer access uses cnp.PyArray_DATA(...) instead of the deprecated
    ndarray.data attribute (identical pointer, supported on NumPy 2).
  * the unused `from skimage._shared.utils import warn` import is dropped.

The segmentation algorithm (edge costs, sorted-edge greedy merge, min_size
post-processing, relabelling) is unchanged, so output is identical to the
original kernel for the same input / library versions.
"""
import numpy as np
...
# ---------------------------------------------------------------------------
# Union-find (copied verbatim from skimage internal _ccomp.pyx)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3D Felzenszwalb segmentation
# ---------------------------------------------------------------------------
def felzenszwalb_cython_3d(image, double scale=1, sigma=0.8,
                           Py_ssize_t min_size=20, spacing=(1, 1, 1)):
    """
    Felzenszwalb's efficient graph based segmentation for single channel 3D
    images. Returns an integer label volume (depth, height, width).
    """
    ...
    cdef cnp.ndarray[cnp.float_t, ndim=1] costs = np.hstack([
        cost1.ravel(), cost2.ravel(), cost3.ravel(), cost7.ravel(), cost9.ravel(),
        cost10.ravel(), cost11.ravel(), cost12.ravel(), cost13.ravel(), cost14.ravel(),
        cost15.ravel(), cost16.ravel(), cost25.ravel()]).astype(float)
    ...
    cdef cnp.ndarray[cnp.intp_t, ndim=2] edges \
            = np.vstack([edges1, edges2, edges3, edges7, edges9, edges10, edges11,
                         edges12, edges13, edges14, edges15, edges16, edges25])
    ...
    cdef cnp.intp_t *segments_p = <cnp.intp_t*>cnp.PyArray_DATA(segments)
    cdef cnp.intp_t *edges_p = <cnp.intp_t*>cnp.PyArray_DATA(edges)
    cdef cnp.float_t *costs_p = <cnp.float_t*>cnp.PyArray_DATA(costs)
    ...
    with nogil:
        ...
        edges_p = <cnp.intp_t*>cnp.PyArray_DATA(edges)
        for e in range(num_costs):
            seg0 = find_root(segments_p, edges_p[0])
            seg1 = find_root(segments_p, edges_p[1])
            edges_p += 2
